...
<div class="container max-w-5xl mx-auto py-8 px-5">
	<div class="pb-10">
		<h3>Offene Stellenanzeigen</h3>
		<p class="relative mt-6 text-md leading-8 text-gray-600 sm:max-w-md lg:max-w-none">
			Wenn du Teil eines Unternehmens sein möchtest, das dir nicht nur einen Job bietet, sondern
			auch echte Möglichkeiten zur persönlichen und beruflichen Entwicklung, dann bewirb dich jetzt
			bei K7. Gemeinsam können wir Großes erreichen und die Zukunft des Sondermaschinenbaus und
			industrieller Technologien gestalten. Wir freuen uns darauf, von dir zu hören!
		</p>
	</div>
	...
	{:then jobs}
		<ul role="list" class="divide-y divide-slate-600/10 pb-10">
			{#each jobs as job}
				<a href={`career/${job?.id}`} data-sveltekit-noscroll>
					<li
